# Programmable completion for the Subversion scm command under bash. Source
# this file (or on some systems add it to ~/.bash_completion and start a new
# shell) and bash's completion mechanism will know all about scm's options!
# Provides completion for the scmadmin command as well.  Who wants to read
# man pages/help text...
...
# val=$(_scm_read_hashfile scm:realmstring < some/file)
function _scm_read_hashfile()
{
...
# _scm_grcut shell-regular-expression
# extract filenames from 'scm status' output
function _scm_grcut()
{
    local re=$1 line=
...
# _scm_lls (dir|file|all) files...
# list scm-managed files from list
# some 'scm status --all-files' would be welcome here?
function _scm_lls()
{
    local opt=$1 f=
    shift
    for f in "$@" ; do
	# could try to check in .scm/entries? hmmm...
...
#   SVN_BASH_KEYWORDS: "scm:keywords" substitutions to be suggested
...
#      suggested for 'revert', only not scm-managed files for 'add', and so on.
#      Possible values are:
#      - username: guess usernames from ~/.subversion/auth/...
#      - scmstatus: use 'scm status' for completion
...
_scm()
{
	...
	cmds='add blame cat checkout co cleanup commit ci \
	      delete del remove rm diff di help list ls log \
		  move mv rename ren revert status stat st update up \
		  make make_package make_deploy refresh clean commit_package status_package diffpackage'
	...
	# scm:* and other (env SVN_BASH_*_PROPS) properties
	local scmProps revProps allProps psCmds propCmds

	# scm and user configured file properties
	scmProps="scm:keywords scm:executable scm:needs-lock scm:externals
	          scm:ignore scm:eol-style scm:mime-type $SVN_BASH_FILE_PROPS"

	# scm and user configured revision properties
	revProps="scm:author scm:log scm:date $SVN_BASH_REV_PROPS"

	# all properties as an array variable
	allProps=( $scmProps $revProps )
	...
	then
		...
		local file=
		# digest? others?
		for file in ~/.subversion/auth/scm.simple/* ; do
		  if [ -r $file ] ; then
		    values="$values $(_scm_read_hashfile username < $file)"
		  fi
		done
	      fi
	      [[ ! "$values" ]] && beep=1
	...
	then
	    ...
	    # where one may get "scm:scm:xxx" in some unclear cases.
	    #
	...
	if [[ $cur != -* || $stat = 'onlyarg' ]]  ; then
	    ...
	    if [[ $SVN_BASH_COMPL_EXT == *scmstatus* ]] ; then

		# build status command and options
		# "--quiet" removes 'unknown' files
		local status='scm status --non-interactive'
		...
		case $cmd in
			...
			files=$($status $cs| _scm_grcut '@([MADR!]*| M*|_M*)')
			;;
		    add) # unknown files
			files=$($status $cs| _scm_grcut '\?*')
			;;
		    unlock) # unlock locked files
			files=$($status $cs| _scm_grcut '@(??L*|?????[KOTB]*)')
			;;
		    resolved) # files in conflict
			files=$($status $cs| _scm_grcut '@(?C*|C*)')
			;;
		    praise|blame|ann*) # any scm file but added
			files=$( _scm_lls all $cur* )
			;;
		    p*) # prop commands
			if [[ $cmd == @($propCmds) && \
			      $prop == @(scm:ignore|scm:externals) ]] ; then
			    # directory specific props
			    files=$( _scm_lls dir . $cur* )
			else
			    # ??? added directories appear twice: foo foo/
			    files="$( _scm_lls all $cur* ) 
                                   $($status $cs | _scm_grcut 'A*' )"
			fi
			;;
		...
		# within subdirectories, so they are added here.
	        # should it be restricted to scm-managed subdirs? no??
		if [[ $SVN_BASH_COMPL_EXT != *recurse* ]] ; then 
		    files="$files $( _scm_lls dir $cur* )"
		fi
...
complete -F _scm -o default -X '@(*/.scm|*/.scm/|.scm|.scm/)' scm
...
_scmadmin ()
{
	...
	cmds='addpackage branch vercopy delpackage refresh clean \
		  make make_package make_deploy confirm diffdeploy help'
...
complete -F _scmadmin -o default scmadmin
